...
$getBasis = new dbOperations($dbConfig,$fieldset_spons,$path,$tableName,$suffix,$salt="I|1Tofu+-Wev>w.W=S\$Y+5$44HC;[<");
...
if ($max_array['max'] <= 0) { // first time here OR button 'new' pressed
	if (!$_POST OR $_POST['submit'] === 'new') {		
		$dbOp = new dbOperations($dbConfig,$fieldset_spons,$path,$tableName,$suffix,$salt="I|1Tofu+-Wev>w.W=S\$Y+5$44HC;[<",$pid,$fid);	
		...
		$self = $selfbase."?id=".$m;		
		$color = "red";
		$form = new createForms($self,$fieldset_spons,$icon);
