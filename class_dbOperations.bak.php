...
class dbOperations {
	...
	private $dbname;
	private $inputArray;
	private $dbEngine = "InnoDB DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci";
	private $salt;
	private $fid;
	...
	function __construct($dbConfig,$inputArray,$relpath=FALSE,$tableName="tx_myForm_",$suffix="001",$salt="I|1Tofu+-Wev>w.W=S\$Y+5$44HC;[<") {
		...
		$sql_connect = new mysqli($dbConfig['dbhost'], $dbConfig['dbuser'], $dbConfig['dbpass']);
	...
	function createTable() { // get db table fields from form's fields names
		...
		$query .= "` (`uid` INT( 11 ) UNSIGNED AUTO_INCREMENT PRIMARY KEY, `crdate` INT( 11 ) UNSIGNED NOT NULL, `tstamp` INT( 11 ), `sid` TINYTEXT NOT NULL,";
		$query .= $fields[0];
		$query .= "`pid` VARCHAR( 255 ) NOT NULL, `fid` INT( 11 )";
		$query .= ") ENGINE = \"".$dbEngine."\";"; 
		...
		die("Cannot connect to database! ") . $sql_connect->error;
			...
			if($sql_connect->query($query) === TRUE) {
				...
				echo "cannot create table" . $sql_connect->error;
			}
		}
	}

	function write2Table($sess=FALSE) { // fid and pid are coming from the powermail form
		$stringOFields = "";
		...
		for ($i=0;$i<$no-1;$i++) {
			...
			if ($this->array_key_check($_POST,$pieces[$i])) {
				...
				$upname = $files[$pieces[$i]];
						...
						$stringmsg .= "<p class=\"errormsg\">".$returnArray[0]."</p>";
						// comma is giood in case it's NOT the last field in form
	...
	function getDataFromDB($scriptname=FALSE, $limit=FALSE, $sort=FALSE, $fields=FALSE, $where=FALSE, $table=FALSE) {
		...
		$query .= $fields;
		$query .= " FROM `".$table."` WHERE `deleted`= 0 AND `hidden`= 0 $where ORDER BY $sort $limit";
		//echo "<p>".$query."</p>";
		$sql_connect = @new mysqli($this->host, $this->user, $this->pass, $this->dbname);
		$sql_connect->set_charset($this->charset);
		if (mysqli_connect_error())
				...
				if($rows > 0) {
					...
					if($result = $sql_connect->query($query)) {
						...
						while($resultcsv = $result->fetch_assoc()) {
							fputcsv($fh, array_values($resultcsv));
						}
						fclose($fh);
						$entries = $rows;
						$filename = $table . ".csv";
						$out = "<div class=\"download\" ><p>Entries: ".$entries."<!-- <br />Rows: ".$rows."<br />Columns: ".$cols." --></p>\n";
						$out .= "<p>Download as csv file: <a href=\"{$filename}\">{$filename}</a></p></div>";
						// end csv
						...
						while ($row = $result->fetch_assoc()) {
							...
							for ($i=0; $i<$cols; $i++) {
								if ($header[$i]==="geb") {
									$format_geb = "%d.%m.%Y";
									$str = $row[$header[$i]];
									$str = strftime($format_geb, $str);
									$html .= "<td class=\"dob\">".$str."</td>";
									...
									$url = $row[$header[$i]];
									$html .= "<td><a href=\"".$this->uploadPath.$url."\">".$url."</a></td>";
								} else {
									if ($i == 0) {
										$id = $row[$header[$i]];
